...
---
# ⚠️ GOVERNED BY AGENTS_CONTRACT.md
...
### Approval Gates Before Agent Spawn
Before spawning ANY agent, CHECK:
- [ ] Dependencies changing? → REQUIRE approval
- [ ] Public API changes? → REQUIRE approval
- [ ] Database schema changes? → REQUIRE approval
- [ ] Deleting code? → REQUIRE approval

IF ANY checked: "⚠️ **APPROVAL REQUIRED**: [operation]. Proceed?"
...
- [ ] Read `tasks/lessons.md` for relevant patterns
- [ ] Check `tasks/todo.md` for pending work
...
- Include 3+ `file:line` citations when available
